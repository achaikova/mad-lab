...
from mad.model.layers.titans.neural_memory import NeuralMemory
...
class SegmentedAttention(Module):
    ...
    def forward_flex(
        ...
        # attention
        out = flex_attn_fn(q.to(torch.bfloat16), k, v)
        out = self.merge_heads(out)
...
class TitansLayer(nn.Module):
    def __init__(
        ...
        **kwargs
        ...
        self.attn = SegmentedAttention(
            ...
            accept_value_residual = not self.is_first,
        ...
        # memory components
        self.mem = None
        self.mem_hyper_conn = None
        self.mem_qkv_layer_selector = None
        
        if is_neural_memory_layer:
            self.mem_hyper_conn = init_hyper_conn(add_branch_out_to_residual = not neural_mem_gate_attn_output)
            
            if not self.is_first and neural_memory_qkv_receives_diff_views:
                num_layer_choices = (layer_index) * 4 + 1
                
                self.mem_qkv_layer_selector = nn.Sequential(
                    nn.RMSNorm(dim),
        ...
        # hyper connections for attention and feedforward
    ...
    def forward(
        ...
        batch, seq_len, neural_mem_segment_len, segment_len, num_longterm_mem_tokens, attn_window_size = *x.shape[:2], self.neural_memory_segment_len, self.segment_len, self.num_longterm_mem_tokens, self.attn_window_size
        ...
        if exists(self.mem):
            ...
            # mem_input, add_residual = self.mem_hyper_conn(x)
            mem_input = x

            if not exists(self.mem_qkv_layer_selector):
                ...
                selected = self.mem_qkv_layer_selector(mem_input)

                qkv_mem_input = einsum(layers_to_choose_from, selected, 'l b n d, v b n l -> v b n d')
            retrieved, next_neural_mem_cache = self.mem.forward(
                qkv_mem_input,
        ...
        attn_out, (values, next_kv_cache) = self.attn(
            attn_in,
        ...
        # ff_in, add_ff_residual = self.ff_hyper_conn(x)
        ff_in = x
